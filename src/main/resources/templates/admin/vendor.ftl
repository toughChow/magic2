    ...
    $(function () {
        ...
        $('a[data-action="close"]').bind('click', function () {
            ...
            layer.confirm('该账号停用后，将不能登录系统，确定要停用?', {
                ...
                J.getJSON('/admin/vendors/close', {id: that.attr('data-id'), active: false}, ajaxReload);
            }, function () {
            });
            return false;
        });
        ...
        // 激活
        $('a[data-action="open"]').bind('click', function () {
            var that = $(this);
            layer.confirm('该账号激活后，将可访问系统中的已授权功能，确定要激活?', {
                btn: ['确定', '取消'], //按钮
                shade: false //不显示遮罩
            }, function () {
                J.getJSON('/admin/vendors/open', {id: that.attr('data-id'), active: true}, ajaxReload);
            }, function () {
            });
            return false;
        });
